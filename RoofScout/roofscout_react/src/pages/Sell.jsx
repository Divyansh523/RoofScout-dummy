...
function Sell() {
  ...
  // Convert selected file to base64 and store in formData.image
  const handleFile = (e) => {
    ...
    reader.onload = () => {
      ...
      setFormData((s) => ({ ...s, image: reader.result }));
      setUploading(false);
    };
    reader.onerror = () => {
      setUploading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Attach ownerName to payload so backend can store it (backend still sets user ref)
      const payload = { ...formData, ownerName };

      const { data } = await API.post("/properties/add", payload);
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl p-8 text-center hover:border-blue-500 dark:hover:border-blue-400 transition-colors">
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-gray-600 dark:text-gray-300 font-semibold mb-2">Click to upload an image</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">JPG, PNG (Max 5MB)</p>
                </div>
              </label>
              {uploading && <div className="text-sm text-blue-600 dark:text-blue-400 mt-4">Processing image…</div>}
              {formData.image && (
                <div className="mt-4">
                  <div className="inline-block relative">
                    <img src={formData.image} alt="preview" className="w-64 h-48 object-cover rounded-xl shadow-lg" />
